                       ...
                       GError      **error)
{
  ...
  g_autoptr(GInputStream) in = NULL;
  const gsize buffer_size = 16 * 1024;
  char buffer[buffer_size];
  gssize size_read;
  gulong current_time;
  gulong last_progress_time;

  g_debug ("Loading %s using libsoup", uri);

  msg = soup_message_new (SOUP_METHOD_GET, uri);
  in = soup_session_send (soup_session, msg, cancellable, NULL);

  /* keep reading the buffer until we get the full response or there is an error */
  for (;;) {
    size_read = g_input_stream_read (in, buffer, buffer_size, cancellable,
                                     error);
    /* check for errors */
    if (size_read == -1)
      return NULL;
    ...
    g_string_append_len (content, buffer, size_read);

    /* report progress */
    current_time = g_get_monotonic_time ();
    if (current_time - last_progress_time > 1 * G_USEC_PER_SEC)
      {
        progress (content->len, user_data);
